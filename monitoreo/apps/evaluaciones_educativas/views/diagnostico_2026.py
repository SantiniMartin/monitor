...
def descargar_excel(request):
	if request.method == 'POST':
		try:
			# 1. Obtener la lista de diccionarios enviada desde el front
			datos = json.loads(request.body) # Esto será una lista: [ {"Nombre": "Juan", ...}, {...} ]
			...
			response['Content-Disposition'] = f'attachment; filename="reporte_diagnostico_2026.xlsx"'
			...
			# #print(alumnos)
			# ws.append(lista)
			columnas = ['CUEANEXO', 'ESCUELA', 'REGION', 'LOCALIDAD', 'DEPARTAMENTO', 'ALUMNOS', 'MATEMATICA', 'LENGUA']
			
			ws.append([]) # Fila vacía de separación
			ws.append(columnas) # Encabezados
			
			# Recorrer la lista enviado desde el front
			for v in datos:
				fila = [v.get(col, "") for col in columnas]
				ws.append(fila)
