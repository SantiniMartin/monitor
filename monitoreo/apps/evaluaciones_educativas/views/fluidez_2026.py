		...
		if form_cueanexo.is_valid():
			...
			if form_grado.is_valid():
				...
				if nombre_grado:
					...
					grado_obj = GradoFluidez2026.objects.filter(nombre_grado=nombre_grado, cueanexo__in=lista_cueanexos)
					#print(len(grado_obj))
...
# Generamos la URL y le pegamos los parámetros del visor
	...
	return render(request, "fluidez_2026/analisis_evaluaciones_junio_2026.html", contexto)
...
# #-----------------------LOGICA PARA SEGUNDO GRADO------------------------
	...
	if type(cueanexo) is str:
		grado=GradoFluidez2026.objects.filter(nombre_grado__icontains=grado_seleccionado, cueanexo=cueanexo)
	else:
		#print('ENTREEE')
		grado=GradoFluidez2026.objects.filter(nombre_grado__icontains=grado_seleccionado, cueanexo__in=cueanexo)
	#print(grado)
	if secciones and turno:
		secciones=SeccionFluidez2026.objects.filter(grado__in=grado, seccion=secciones,turno=turno)
	#print(seccion)
	else:
	...
	evaluaciones = EvaluacionFluidezLectoraFluidez2026.objects.filter(alumno__in=alumnos)
	#comprension lectora
...
def comprension_lectora(alumnos,grado_seleccionado):
	...
	evaluaciones=EvaluacionFluidezLectoraFluidez2026.objects.filter(alumno__in=alumnos, asistencia='PRESENTE')
